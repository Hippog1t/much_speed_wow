...
def update_state( new_state, target_tab, letter_sent_to_arduino ):
    ...
    speed_in_bytes = bytes(str(int(speed_in_bytes)), 'utf-8')
    #print("speed in bytes ", speed_in_bytes)
    rot_in_bytes = tab[1]
    rot_in_bytes = bytes(str(int(rot_in_bytes)), 'utf-8')
    #print("Rot in bytes ", rot_in_bytes)
# insert sleep HERE
...
while True:
    ...
    # retardation case down+left+right
        ...
        update_state(  new_state = "down" , target_tab = [ BACKWARD_SPEED , NEUTRAL ] ,  letter_sent_to_arduino = "A")
